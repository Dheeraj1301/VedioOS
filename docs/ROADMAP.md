...
**2026-09-24: configuration-audit and deployment-health foundations verified.** `check_release` fails closed on insecure runtime/provider combinations, supports secret-free JSON for CI and lists manual policy/recovery gates. Generic liveness and database/migration readiness probes are available for a future orchestrator. The current development environment intentionally fails the release audit; no deployment is authorized. See [Phase 9 evidence](PHASE_9.md).
