...
- Client email verification with expiry, resend throttling and a replaceable delivery backend. Production email provider/domain configuration remains required before launch.
- Four order-choice blocks: the three administrator-configured plans and one customization path. Unconfigured plans remain unavailable rather than displaying invented prices.
- Custom fields for colour grading, quality enhancement, reel-duration bands, wording, and wording/font direction, plus a separate private font-inspiration upload.
...
- The isolated suite passes 102 tests with 10 opt-in integrations skipped.
